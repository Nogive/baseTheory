...
myApp.onPageInit("login", function (page) {
  $$("#goLogin").on("click", function () {
    ...
    if (isValid) {
      myApp.showPreloader();
      questRandomCode(account).then(function (data) {
        var salt = data.salt;
        var randomCode = data.randomCode;
        var passwordSignedWithSalt = calculateRFC2104HMAC(password, salt);
        var randomCodeSha1Hash = CryptoJS.SHA1(randomCode);
        var signedPassword = calculateRFC2104HMAC(CryptoJS.enc.Hex.stringify(passwordSignedWithSalt), randomCodeSha1Hash);
        var params = {
          account: account,
